...
public class playerCacheFeature {
		...
		ServerLifecycleEvents.SERVER_STARTED.register(( server ) -> {
			if( Platform.CONFIG.enablePlayerCaching ) {
				...
				if( Platform.CONFIG.flushCacheOnServerRestart ) {
					emptyCache( server );
				} else if( !serverState.players.isEmpty() ) {
	...
	public static ProfileResultData cachePlayer( String playerName, UUID playerUuid, ProfileResult profileResult ) {
		...
		Platform.LOGGER.info( "Attempted to cache player but ProfileResult is missing: {} ({})", playerName, playerUuid.toString() );
	...
	public static ProfileResultData addToCache(UUID uuid, ProfileResultData data ) {
		if( null == data.result ) {
			Platform.LOGGER.info( "Attempted to add player to cache but ProfileResult is missing: {} ({})", data.name, data.uuid.toString() );
			return data;
		} // if
	...
	public static void emptyCache( MinecraftServer server ) {
		PersistentProfileResultData serverState = PersistentProfileResultData.getServerState(server);
		
		serverState.players = new HashMap<>();
